...
import {usePermissionStore} from '@/common/scripts/store/modules/permission'
...
async function registRouter(appStore, to, next) {
  if (storage.getItem('menusType') == '0') {
    ...
    // if (!curSYSName) curSYSName = 'basicPlatform'
    if (curSYSName) {
      const permissionStore = usePermissionStore()
      if (permissionStore.GetSystemUpdate(curSYSName)) {
        next()
      } else {
        await getChildData(curSYSName).then(async res => {
          permissionStore.GenerateRoutesByMenus({sysName: curSYSName, router: res.default.router}).then(() => {
            next({...to})
          })
        }).catch(err => {
